...
const std::string pubFile  = "/tmp/publickey";
const std::string secFile  = "/tmp/secretkey";
const std::string signFile = "/tmp/sign";
...
int verify(const std::string& m, int id, std::string pubStr, std::string signatureStr)
{
	printf("verify message `%s` by id=%d\n", m.c_str(), id);
	bls::PublicKey pub;
	load(pub, pubFile, id);
        if (pubStr != "") {pub.setStr(pubStr);}
	bls::Signature s;
	load(s, signFile, id);
        if (signatureStr != "") {s.setStr(signatureStr);}
	if (s.verify(pub, m)) {
		puts("verify ok");
		return 0;
	} else {
	...
	try
{
	...
	opt.appendOpt(&pub, "", "p", ":publicKey");
	opt.appendOpt(&signature, "", "s", ":signature");
	...
	if (mode == "init") {
		...
		if (m.empty()) goto ERR_EXIT;
		return verify(m, id, pub, signature);
	} else if (mode == "share") {
		return share(n, k);
